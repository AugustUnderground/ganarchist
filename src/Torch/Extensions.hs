...
linalgSolve x y = T.linalg_solve x y True
...
-- | same as `select'` but works on gpu
select'' :: Int -> Int -> T.Tensor -> T.Tensor
...
pdf :: (T.Scalar s, Floating s) => s -> s -> T.Tensor -> T.Tensor
...
intIdx set = fromJust . mapM Just . mapMaybe (`elemIndex` set)
